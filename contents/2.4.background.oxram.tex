...
(HRS)~\cite{wong2012metal, chen2020reram}. A diagram for the state of
the conductive fillament under a bipolar switching regime is shown in
Figure \ref{fig:oxram-prog}. A SET operation applies a positive voltage
across the device, driving oxygen ions out of the switching layer and
leaving behind a chain of oxygen vacancies that form a conductive
filament (CF) bridging the two electrodes~\cite{wei2008highly,
	yu2012switching}. The completed filament is the LRS. Note that as the
distance between the filament gap is closed, . A RESET operation applies a voltage
of the opposite polarity, which drives oxygen ions back into the
filament region, rupturing the CF and returning the device to the
