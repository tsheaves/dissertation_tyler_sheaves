...
in the same word~\cite{xu2015overcoming}. As a result, write controllers
must serialize SET and RESET phases: all cells requiring a SET
transition within a word are programmed in one phase, and all cells
requiring a RESET transition are programmed in a separate phase. A write
operation that requires both polarities within a single word therefore
incurs the latency of two sequential programming phases, while a write
requiring only one polarity incurs a single phase.
